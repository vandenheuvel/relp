    ...
    for<'r> &'r OF: OrderedFieldRef<OF>,
{
    ...
    fn after_bound_change(
        ...
        debug_assert!(match direction {
            BoundDirection::Lower => change.as_ref().map_or(true, NonZeroSigned::non_zero_is_positive),
            BoundDirection::Upper => change.as_ref().map_or(true, NonZeroSigned::non_zero_is_negative),
        });
