...
pub trait PivotRule<TT: TableauType> {
    ...
    fn select_primal_pivot_column<'a, OF, OFZ, MP>(
        &mut self,
...
impl PivotRule<Artificial> for FirstProfitable {
    ...
    fn select_primal_pivot_column<'a, OF, OFZ, MP>(
        &mut self,
        tableau: &Tableau<'a, OF, OFZ, Artificial, MP>,
    ) -> Option<SparseTuple<OF>>
    where
        OF: OrderedField,
        for<'r> &'r OF: OrderedFieldRef<OF>,
...
impl PivotRule<NonArtificial> for FirstProfitable {
    ...
    fn select_primal_pivot_column<'a, OF, OFZ, MP>(
        &mut self,
        tableau: &Tableau<'a, OF, OFZ, NonArtificial, MP>,
    ) -> Option<SparseTuple<OF>>
        where
            OF: OrderedField,
            for<'r> &'r OF: OrderedFieldRef<OF>,
...
impl PivotRule<NonArtificial> for FirstProfitableWithMemory {
            ...
            MP: MatrixProvider<OF, OFZ>,
    {
        let to_consider = if let Some(last) = self.last_selected {
            ...
            (0..tableau.nr_columns()).chain(0..0) // TODO: Avoid this empty chain.
        };
