...
pub struct LUDecomposition<F> {
    ...
    ///
    /// Column major, diagonal is excluded and not necessarily ones. Length is `m - 1`.
    ...
    F: ops::Field + ops::FieldHR,
{
    ...
    fn change_basis(
        ...
        *self.upper_diagonal.last_mut().unwrap() = corner_value;
    ...
    fn basis_inverse_row(&self, mut row: usize) -> SparseVector<Self::F, Self::F> {
        ...
        let mut w = self.right_multiply_by_upper_inverse(initial_rhs);
        ...
        fn from_5x5_identity() {
            ...
            let mut initial = LUDecomposition {
                ...
                upper_triangular: vec![
                    ...
                    vec![(0, RB!(15)), (1, RB!(25)), (2, RB!(35)), (3, RB!(45))],
                ],
            ...
            let expected = LUDecomposition {
                ...
                upper_triangular: vec![
                    ...
                    vec![(0, RB!(12)), (1, RB!(32)), (2, RB!(42))],
                ],
                upper_diagonal: vec![RB!(11), RB!(33), RB!(44), RB!(55), RB!(-215, 363)],
                updates: vec![(
                    EtaFile::new(vec![
                            (2, RB!(23, 33)),
