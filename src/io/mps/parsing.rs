...
impl<'a, F: Clone> TryFrom<Vec<(FileLocation<'a>, Vec<Atom<'a, F>>)>> for UnstructuredMPS<'a, F> {
    ...
    fn try_from(
        ...
        // Each section appears at most once. Keep track of which section currently is being parsed.
        let mut current_section: Option<Section> = None;

        // Collecting the program name (right at the start).
        let mut name = None;
        // Collecting the cost row name (typically the first row).
        ...
        for (file_location, line) in atom_lines {
            // See if a new section has started.
...
/// # Arguments
///
/// * `line`: The tokens on the line of the problem file.
/// * `cost_row_name`: Potentially previously parsed name of the cost row.
/// * `row_collector`: Data structure in which to collect all parsed rows that are not the cost row.
///
/// # Return value
///
